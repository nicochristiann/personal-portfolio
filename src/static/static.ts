export const navLinks = [
  { href: "#about", label: "About" },
  { href: "#projects", label: "Projects" },
  //   { href: "#experience", label: "Experience" },
  { href: "#contact", label: "Contact" },
];

export const projects = [
  {
    title: "SPIN CMS",
    description:
      "A web-based content management system engineered to efficiently manage, structure, and deliver regional industrial and spatial datasets through a scalable, data-driven interface.",
    image: "/spin.png",
    tags: ["Nextjs", "Tailwind CSS", "Shadcn UI", "PostgreSQL"],
    link: "",
    notes: "Company Team Project",
    github: "",
  },
  {
    title: "Expenze",
    description:
      "A personal finance app that manages 100+ transactions across multiple payment methods, enabling users to monitor expenses and income in one place.",
    image: "/expenze.png",
    tags: ["React", "TypeScript", "Tailwind CSS", "Neon DB", "Drizzle ORM"],
    link: "https://my-expenze.vercel.app/",
    notes: "Personal Project",
    github: "https://github.com/nicochristiann/expenze",
  },
  {
    title: "Foster Legal",
    description:
      "A responsive, high-performance law firm website engineered to present legal services clearly and professionally, with a focus on usability, accessibility, and modern web standards.",
    image: "/foster.png",
    tags: ["Webflow", "JavaScript", "Figma"],
    link: "https://fosterlegal.co.id/",
    notes: "Company Solo Project",
    github: "",
  },
  {
    title: "KPAII Website",
    description:
      "A data-driven government information platform developed to manage and visualize regional industrial data, featuring dynamic content management, interactive maps, and scalable front-end architecture for public access and transparency.",
    image: "/kpaii.png",
    tags: ["Nextjs", "Tailwind CSS", "Shadcn UI", "Directus CMS"],
    link: "https://kpaii.kemenperin.go.id/id",
    notes: "Company Team Project",
    github: "",
  },
];

export const aboutParagraphs = [
  {
    title: "Who Am I?",
    paragraph:
      "I’m a frontend developer and university student with a strong interest in building modern, reliable web applications. I enjoy turning ideas and requirements into well-structured interfaces that are easy to use, visually clear, and technically sound.",
  },
  {
    title: "What Am I Specialize In?",
    paragraph:
      "I specialize in frontend development with a focus on component-based architecture, responsive design, and performance optimization. My work primarily involves technologies such as React, modern CSS frameworks, and JavaScript ecosystems to create scalable and maintainable user interfaces.",
  },
  {
    title: "How My Skills Create Impact?",
    paragraph:
      "By combining technical precision with user-centered thinking, I build solutions that improve usability, reduce complexity, and support real-world use cases. My skills help transform complex problems into intuitive digital experiences that deliver measurable value to users and stakeholders.",
  },
];
